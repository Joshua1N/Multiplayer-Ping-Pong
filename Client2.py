...
class Server:
    ...
    def openServer(self):
        # When player runs command : openserver
...
class Client:
    ...
    def connect(self):
        # When player runs command : connect
        while True:
...
def Prompt():
    # Prompt player to openserver or connect to existing server
    connection = Server()
    hostName = socket.gethostname()
    print(f"Hi {hostName}, are you hosting a game or connecting to an existing one?\n 'Help' for a list of commands.")
    while True:
...
class Ball:
    ...
    def isCollision(self, player, competitor, top, bottom, left, right):
        if self.location[0] <= left + self.radius:  # collision with left edge of screen
            self.location = [self.WIDTH // 2, self.HEIGHT // 2]  # Give ball back to who scored
...
class Player:
    def __init__(self, surface):
        # Drawing the paddles / handling position
...
class Competitor:
    ...
    def handleRequest(self, client):
        # Get requests from other client
        try:
...
def winCondition(player_point, competitor_point, host):
    if player_point == 5:
        print(f"You win! Final Score: {player_point} : {competitor_point}")
        host.close()
        pygame.quit()
    elif competitor_point == 5:
        print(f"You Lose! Final Score: {competitor_point} : {player_point}")
        host.close()
        pygame.quit()
...
class PingPong:
    ...
    def renderScoreBoard(self, player_point, competitor_point):
        ...
        font = os.path.join(PingPong.init_directory,
                            "C:/Users/joshu/Downloads/pingponggame-main/pingponggame-main/source/cour.ttf")
        size = 48
    ...
    def start(self):
        ...
        while not gameOver:
            ...
            winCondition(player.point, competitor.point, host)
